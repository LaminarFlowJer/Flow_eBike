...
In the Python-only SIL build, the firmware tick rate is the effective sensor
read rate. Verify it >= 10 Hz over a 1 s window by counting unique sample
timestamps. (Real hardware will be re-tested at HIL.)
"""
import pytest
...
def test_torque_publish_rate(harness, rider):
    ...
    rate_hz = n_samples / 1.0  # 1 s window
